...
class Train(core.DeepFinder):
    ...
    def launch(self, path_train, path_valid=None):
        ...
        plotting_callback = callbacks.TrainingPlotCallback(validation_data=initial_valid_dataset, validation_steps=self.steps_per_valid, path_out=self.path_out, label_list=self.label_list)
    ...
    def create_tf_dataset(self, copick_path, input_dataset, input_target, batch_size, dim_in, Ncl, flag_batch_bootstrap = True, tomoIDs = None):

        copickRoot = CopickRootFSSpec.from_file(copick_path)
        organizedPicksDict = core.query_available_picks(copickRoot, tomoIDs)
        dataset = tf.data.Dataset.from_generator(
            lambda: self.copick_data_generator(input_dataset, input_target, batch_size, dim_in, Ncl, flag_batch_bootstrap, organizedPicksDict),
            output_signature=(
    ...
    def copick_data_generator(self, input_dataset, input_target, batch_size, dim_in, Ncl, flag_batch_bootstrap, organizedPicksDict):
        ...
        while True:
            ...
            for i in range(batch_size):
                ...
                patch_data = input_dataset[pool['tomoID_idx'][index]][z-p_in:z+p_in, y-p_in:y+p_in, x-p_in:x+p_in]
                patch_target = input_target[pool['tomoID_idx'][index]][z-p_in:z+p_in, y-p_in:y+p_in, x-p_in:x+p_in]
