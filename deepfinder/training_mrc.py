...
from deepfinder import losses, models
...
class Train(core.DeepFinder):
    def __init__(self, Ncl, dim_in):
        ...
        self.path_out = "./"
        self.h5_dset_name = (
            "dataset"  # if training set is stored as .h5 file, specify here in which h5 dataset the arrays are stored
        )
    ...
    def launch(self, path_data, path_target, objlist_train, objlist_valid):
        ...
        #
        # # Load whole dataset:
        # if self.flag_direct_read is False:
    ...
    def data_generator(self, data, target, objlist, batch_size, dim_in, Ncl, flag_batch_bootstrap):
        ...
        while True:
            pool = core.get_bootstrap_idx(objlist, Nbs=batch_size) if flag_batch_bootstrap else range(0, len(objlist))
